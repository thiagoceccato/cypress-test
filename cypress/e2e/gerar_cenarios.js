// cypress/e2e/gerar_cenarios.js
// Gera arquivos - LÓGICA EXATA DO ARQUIVO QUE FUNCIONOU + ESCRITA EM TEMPO REAL

const fs = require('fs');
const path = require('path');
const { CENARIOS } = require('./config/cenarios.js');

// Template baseado EXATAMENTE no auditoria_seja_associado.cy.js que FUNCIONOU
const template = (nome, url) => `// Auto-generated - ${nome}
const FLUXO = '${nome}';
const URL_PAGE = '${url}';
const CSV = 'C:/temp/sicredi-audit/${nome}.csv';
...
// Cria pasta
const dir = path.join(__dirname, 'paralelo');
if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });

// Limpa arquivos antigos
fs.readdirSync(dir).filter(f => f.endsWith('.cy.js')).forEach(f => {
  fs.unlinkSync(path.join(dir, f));
});

// Gera novos
CENARIOS.forEach(c => {
  const fileName = c.nome.replace(/[^a-z0-9-]/gi, '-') + '.cy.js';
  fs.writeFileSync(path.join(dir, fileName), template(c.nome, c.url));
  console.log('✓ ' + fileName);
});

console.log('\n✅ ' + CENARIOS.length + ' cenários gerados');
