// GRUPO 4: pix + outros (24 URLs)
const { parseGaUrl } = require('./utils/gaHelpers');
const { CENARIOS } = require('./config/cenarios');

const GRUPO = 4;
const FILTROS = ['pix', 'solucoes-pj', 'mei', 'conta-corrente', 'pagamentos', 'home', 'indicacao-mgm', 'cambio-e-exterior', 'campanhas-institucionais', 'catalogo-de-produtos', 'programas-e-iniciativa', 'energia-solar', 'conteudo-educacao', 'open-finance', 'institucional', 'seja-associado'];
...
describe(`GRUPO ${GRUPO} - Outros (${CENARIOS_GRUPO.length} URLs)`, () => {
  before(() => {
